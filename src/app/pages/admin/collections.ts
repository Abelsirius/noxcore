  ...
  template: `
    <div class="space-y-8">
      ...
      <div *ngIf="isLoading()" class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div *ngFor="let i of [1,2,3]" class="bg-white/5 border border-white/10 aspect-[16/10] shimmer"></div>
      </div>

      <div *ngIf="!isLoading()" class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <!-- Collection Cards -->
        <div *ngFor="let col of collections" class="relative bg-[#111] border border-white/10 group hover:border-red-600 transition-all overflow-hidden aspect-[16/10]">
          <img *ngIf="col.image_url" [src]="col.image_url" class="absolute inset-0 w-full h-full object-cover opacity-40 group-hover:opacity-60 transition-opacity">
        ...
        <div *ngIf="!isAdding" (click)="isAdding = true" 
        ...
        <!-- New Collection Form -->
        <div *ngIf="isAdding" class="relative bg-[#0a0a0a] border border-red-600/50 p-6 space-y-4 aspect-[16/10] overflow-hidden">
          <!-- Background Preview -->
          <img *ngIf="newCol.image_url" [src]="newCol.image_url" class="absolute inset-0 w-full h-full object-cover opacity-20">
          
          <div class="relative z-10 h-full flex flex-col justify-center space-y-4">
            <input type="text" [(ngModel)]="newCol.name" placeholder="NOMBRE DE COLECCIÓN" 
            ...
            <select [(ngModel)]="newCol.home_section" 
                    class="w-full bg-black/80 border border-white/10 text-white px-4 py-2 text-[10px] font-bold tracking-widest uppercase focus:border-red-600 outline-none">
            ...
            <div class="flex items-center gap-2">
              <button (click)="fileInput.click()" [disabled]="isUploading() || isSaving()" 
                ...
                <i *ngIf="isUploading()" class="ti ti-loader-2 animate-spin mr-2"></i>
            ...
            <div class="flex gap-2">
              <button (click)="saveCollection()" [disabled]="!newCol.name || isUploading() || isSaving()"
                ...
                <i *ngIf="isSaving()" class="ti ti-loader-2 animate-spin mr-2"></i>
...
export class AdminCollectionsComponent implements OnInit {
  ...
  async onFileSelected(event: any) {
    ...
    try {
      ...
      this.newCol.image_url = data.publicUrl;
    } catch (e: any) {
  ...
  async saveCollection() {
    if (!this.newCol.name || this.isSaving()) return;
    
    this.isSaving.set(true);
    const slug = this.newCol.name.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '');
    
    // Usamos el campo descripción para guardar el metadato de la sección
    const description = `SECTION:${this.newCol.home_section}|${this.newCol.name}`;

    try {
      const { error } = await this.supabase
        .from('collections')
        .insert([{ 
          name: this.newCol.name, 
          slug: slug,
          image_url: this.newCol.image_url,
      ...
      this.isAdding = false;
      this.newCol = { name: '', slug: '', image_url: '', home_section: 'none' };
      ...
      setTimeout(() => {
        ...
        if (!this.isAdding) {
          // Si se guardó correctamente, isAdding es false
        }
      }, 500);
    }
  }
