...
@Component({
  ...
  template: `
    <section class="relative overflow-hidden py-20 px-4" aria-labelledby="testimonials-title">
      ...
      <div class="max-w-6xl mx-auto text-center mb-12">
        ...
        <p class="mt-4 text-sm text-gray-300 max-w-2xl mx-auto"> Compra verificada y entrega.</p>
      </div>

      <div class="max-w-6xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        <article
          *ngFor="let t of testimonials"
          class="testimonial-card transform transition-all duration-300"
          [attr.data-id]="t.id"
          #card
        >
              ...
              <div class="text-left">
                <div class="flex items-center gap-2">
                  ...
                  <span *ngIf="t.verified" class="ml-1 text-xs inline-flex items-center gap-1 text-gray-300">
                    ...
                    <span class="text-xs">Cliente satisfecho</span>

                  </span>
                </div>
                <div class="flex items-center mt-1">
                  <ng-container *ngFor="let i of [1,2,3,4,5]; let idx = index">
                    <svg *ngIf="idx < t.rating" class="w-4 h-4 text-red-500" viewBox="0 0 20 20" fill="currentColor"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.286 3.95a1 1 0 0 0 .95.69h4.154c.969 0 1.371 1.24.588 1.81l-3.36 2.44a1 1 0 0 0-.364 1.118l1.286 3.95c.3.921-.755 1.688-1.54 1.118l-3.36-2.44a1 1 0 0 0-1.176 0l-3.36 2.44c-.785.57-1.84-.197-1.54-1.118l1.286-3.95a1 1 0 0 0-.364-1.118L2.07 9.377c-.783-.57-.38-1.81.588-1.81h4.154a1 1 0 0 0 .95-.69l1.286-3.95z"/></svg>
                    <svg *ngIf="idx >= t.rating" class="w-4 h-4 text-gray-700" viewBox="0 0 20 20" fill="currentColor"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.286 3.95a1 1 0 0 0 .95.69h4.154c.969 0 1.371 1.24.588 1.81l-3.36 2.44a1 1 0 0 0-.364 1.118l1.286 3.95c.3.921-.755 1.688-1.54 1.118l-3.36-2.44a1 1 0 0 0-1.176 0l-3.36 2.44c-.785.57-1.84-.197-1.54-1.118l1.286-3.95a1 1 0 0 0-.364-1.118L2.07 9.377c-.783-.57-.38-1.81.588-1.81h4.154a1 1 0 0 0 .95-.69l1.286-3.95z"/></svg>
                  </ng-container>
                </div>
              </div>
            </div>
  <img [src]="t.image" [alt]="t.name" class="w-full h-[250px] rounded-lg object-cover border-2 border-gray-800 shadow-sm"/>
            <p class="text-gray-300 text-sm leading-relaxed mt-3 mb-4 line-clamp-3">“{{ t.text }}”</p>

            <div class="mt-auto flex items-center justify-between">
              <span class="text-xs text-gray-500">Hace {{t.agoDay }} días</span>
  ...
  styles: [`
    ...
    /* util para line-clamp si no usas plugin tailwind */
...
export class TestimonialsComponent implements OnInit {
  testimonials: Testimonial[] = [
    {
      id: 't1',
      name: 'Carlos M.',
      text: 'La tela es brutal, ajuste perfecto y la entrega llega. Repetiré sin duda.',
      rating: 5,
      image: "../../../../assets/temo1.jpeg",
      verified: true,
      agoDay: 2,
    },
  ...
  // helper para demo: genera días aleatorios
}